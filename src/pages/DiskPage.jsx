...
import { fetchOneDisk } from '../http/productAPI';
import { useParams } from 'react-router-dom';

const DiskPage = ({ setActive }) => {
  const [disk, setDisk] = React.useState({ info: [] });
  const { id } = useParams();
  React.useEffect(() => {
    fetchOneDisk(id).then((data) => setDisk(data));
  }, []);
  const goToTop = () => {
    window.scrollTo(0, 0);
  };
        ...
        <div className="product-page__inner">
          <div className="product-page__top">
            ...
            <div className="product-page__top-right">
              <p className="product-page__name">{disk.name}</p>
              <span className="product-page__price">{disk.price} р</span>
              <button className="btn btn--gray" onClick={() => setActive(true)}>
                Связаться
              </button>
          ...
          <ul className="product-page__bottom">
            <h3 className="product-page__bottom-title">
              Характеристики: <span>{disk.name}</span>
            </h3>
            {disk.info.map((info, index) => (
              <li
